...
def calculate_expected_payoffs():
    ...
    for response in ["A", "F"]:
        # 计算新进入者和现有竞争者的期望收益
        new_entrant_payoff_L, competitor_payoff_L = specific_payoffs[f"E_{response}_L"]
        new_entrant_payoff_H, competitor_payoff_H = specific_payoffs[f"E_{response}_H"]
        expected_payoffs[f"E_{response}"] = {
            "New Entrant Payoff": p_L * new_entrant_payoff_L
            + p_H * new_entrant_payoff_H,
            "Competitor Payoff": p_L * competitor_payoff_L + p_H * competitor_payoff_H,
        }

    # 不进入市场的情况
    expected_payoffs["NE"] = {
        "New Entrant Payoff": 0,  # 新进入者选择不进入市场的收益为0
        "Competitor Payoff": p_L * specific_payoffs["NE"][1],  # 现有竞争者的收益
    }
...
def print_best_strategy(expected_payoffs):
    ...
    if best_strategy == "NE":
        ...
        print(f"Best strategy for the New Entrant: {best_strategy.split('_')[1]}")
...
def find_nash_equilibrium(expected_payoffs):
    ...
    for strategy, payoffs in expected_payoffs.items():
        ...
        if new_entrant_payoff >= expected_payoffs["NE"]["New Entrant Payoff"]:
            ...
            if competitor_payoff >= max(
                expected_payoffs["E_A"]["Competitor Payoff"],
                expected_payoffs["E_F"]["Competitor Payoff"],
                expected_payoffs["NE"]["Competitor Payoff"],
            ):
